  ...
  <div>
    <v-slide-y-transition>
      ...
      <v-container fluid v-if="mounted">
        <!--<v-btn @click="getViolationRule">getList</v-btn>-->
              ...
              <v-card-title class="py-2">
                ...
                <v-breadcrumbs class="py-1 my-2" large>
                  ...
                  <v-breadcrumbs-item
                    ...
                    :key="item.id"
                  >
                    <span @click="goChildren(item, -1)">{{ item.name }}</span>
                  </v-breadcrumbs-item>
                </v-breadcrumbs>
                <v-spacer></v-spacer>
                <!--<v-btn-->
...
export default {
  components: {ConfirmOerationDialog},
  data: () => ({
    loading: false,
    mounted: false,
    name: 'ViolationTypes',
  ...
  methods: {
    ...
    async getViolationRule () {
      ...
      if (this.bcItems.length > 1) {
        let last = this.bcItems[this.bcItems.length - 1]
        route = last.code.split('_').map(v => {
          v = Number(v)
          v -= 1
          return v
        })
  ...
  created () {
    this.getViolationRule().then(() => {
      // this.bcItems.push(BCROOT)
    })
  },
  mounted () {
    this.mounted = true
