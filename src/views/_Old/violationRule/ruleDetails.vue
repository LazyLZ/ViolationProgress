              ...
              <v-toolbar-items>
                <v-flex layout align-center>
                  <template v-for="(item, i) in ruleStack.slice(1)">
                    ...
                    <v-icon :key="item.id + 'icon'" v-if="i < ruleStack.length - 2" small class="px-2">chevron_right
                    </v-icon>
                  </template>
                    ...
                    <v-layout wrap>
                      ...
                      <v-flex xs6>
                        <v-text-field
                          ...
                          prepend-icon="report"
                          type="number"
                          hint="严重程度高的规则将累计到严重程度低的规则次数"
                          :disabled="!rootRule"
                          v-model="severityDegree"
                        ></v-text-field>
                      </v-flex>
                      <!--<v-flex xs12>-->
                      <!--<v-switch-->
                                ...
                                <v-list-tile-content>
                                  ...
                                  <v-list-tile-sub-title>
                                    <v-tooltip top max-width="56em" open-delay="600">
                                      <span slot="activator">{{template.text || '未设置'}}</span>
  ...
  methods: {
    ...
    async getGroupList () {
      ...
      if (!this.violationRule.orgId) {
        ...
        try {
          let {items, amount} = await this.$store.dispatch('useradmin/getGroups', {
          ...
          amount = amount + 1
          console.log('get groups', items, amount)
          this.groupInfos = items
        }
        catch (e) {
      ...
      if (this.allChecked) {
        ...
        try {
          if (this.editing) {
            ...
            this.$store.commit('toggleSnackbar', {message: '修改成功'})
          }
            ...
            this.$store.commit('toggleSnackbar', {message: '添加成功'})
          }
          this.cancel()
        }
        catch (e) {
    ...
    async addDetection () {
      ...
      for (let g of Object.keys(this.groupTemplateList.message)) {
        if (this.groups.indexOf(g) < 0) continue
    ...
    async addRule () {
      ...
      if (this.rootRule) {
        data.severityDegree = Number(this.severityDegree)
      }
  ...
  async created () {
    ...
    if (this.editing || this.readonly) {
      ...
      if (rule) {
        ...
        this.severityDegree = Number(this.violationRule.rootRule.severityDegree)
    ...
    else if (!this.ruleStack || this.ruleStack.length < 1) {
      this.cancel()
    }
    else {
      this.pageLoading = true
      if (!this.rootRule) {
        this.severityDegree = this.ruleStack[1].severityDegree
      }
